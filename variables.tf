...
variable "middle_name" {
  description = "The middle name of Elasitcache"
  type        = string
  default     = null
}
...
variable "subnet_ids" {
  type        = list(string)
  description = <<-EOT
    subnet ids for Elasticache
    subnet_ids = [ "subnet-0f15980c684", ]
    EOT
}
...
variable "alarm_actions" {
  type        = list(string)
  description = "The list of actions to execute when this alarm transitions into an ALARM state from any other state. Each action is specified as an Amazon Resource Name."
  default     = []
}
...
variable "security_group_ids" {
  type        = list(string)
  description = "A list of IDs of Security Groups to associate the created resource with it."
  # default     = []
}

variable "additional_tags" {
  description = "Additional tags for the ElastiCache"
  type        = map(string)
  default     = {}
}
