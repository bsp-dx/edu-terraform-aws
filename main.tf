locals {
  create_lt = length(var.name) > 1 ? true : false
  lt_name   = "${var.context.name_prefix}-${var.name}-lt"
  name      = "${var.context.name_prefix}-${var.name}"

  iam_name_prefix           = format("%s%s%s", var.context.project, upper(substr(var.name, 0, 1)), lower(substr(var.name, 1, length(var.name))))
  iam_instance_profile_name = var.create_iam_instance_profile && var.iam_instance_profile_name == null ? format("%sEC2InstanceProfile", local.iam_name_prefix) : var.iam_instance_profile_name

  tags = var.context.tags
}

module "inner_ec2_profile" {
  source                      = "./instance-profile"
  create_iam_instance_profile = var.create_iam_instance_profile
  iam_instance_role_name      = format("%sEC2InstanceRole", local.iam_name_prefix)
...
resource "aws_launch_template" "this" {
  ...
  dynamic "enclave_options" {
    for_each = var.enclave_options != null ? [1] : []
    content {
      enabled = var.enclave_options
    }
  }

  dynamic "hibernation_options" {
    for_each = var.hibernation_options != null ? [1] : []
    content {
      configured = var.hibernation_options
    }
  }
  ...
  tag_specifications {
    ...
    tags = merge(local.tags, var.instance_tags, { Name = local.name })
  }
